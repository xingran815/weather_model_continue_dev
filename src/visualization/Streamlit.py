...
pages = [
    ...
    "Prediction :chart_with_upwards_trend:",
    "Conclusion:grey_exclamation:",
]
...
if page == pages[0]:
    ...
    st.write(
        "The future weather is an important information for lots of areas of life e.g. agriculture or leisure activities"
    )
    st.write("The aim of the project is to predict if it is raining tomorrow based on today weather.")

    st.subheader("Dataset")
    st.write(
        "The given Dataset from Australia has",
        df.shape[1],
        "different columns and contains ",
        df.shape[0],
        " data entrys. The target column is RainTomorrow, which is a Boolean. ",
    )
    st.dataframe(df)
    ...
    st.write(
        "If it rains today, there is 50% chance that it also rains tomorrow. If it does not rain today, "
...
if page == pages[2]:
    ...
    st.write(
        'Pressing the button "Make dataset" chooses a random percentage of the original data to create a new dataset'
    )
...
if page == pages[5]:
    ...
    st.write(
        "The FastAPI application is utilized to expose the core functionalities of the machine learning project:\n"
        "- making dataset,\n"
        "- preprocessing,\n"
        "- model training,\n"
        "- model prediction.\n"
    ...
    st.write(
        "Generally speaking, making dataset and preprocessing are fast to finish,\n"
    ...
    st.write(
        "- The project does predict the weather of tomorrow by using changing data automatically\n"
    ...
    st.write(
        "When we had more time for the project we would extend it with:\n"
